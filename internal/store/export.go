	...
	sources := []export.RowSource{
		...
		&querySource{tx: tx, table: "agent_email_realm_aliases", q: `
			SELECT jsonb_build_object(
			  ...
			  'realm_id', realm_id, 'domain', domain,
			  'realm_label', realm_label, 'state', state,
		...
		&querySource{tx: tx, table: "agent_email_messages", q: `
			SELECT jsonb_build_object(
			  ...
			  'body_text', body_text, 'body_text_kind', body_text_kind,
			  'attachment_storage_bytes', attachment_storage_bytes,
			  'retained_attachment_storage_bytes', retained_attachment_storage_bytes,
