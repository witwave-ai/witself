...
func emptySummary(now time.Time) Summary {
	now = now.UTC()
	s := Summary{
		Schema: "witself.agent-summary.v1", GeneratedAt: now, RefreshAfterSeconds: 30,
...
func summaryDefinitions() []summaryDefinition {
	return []summaryDefinition{
		{key: "transactions", label: "Transactions", code: "OPS", inventory: "records"},
		...
		{key: "memories", label: "Memories", code: "MEM", inventory: "active memories", action: "memory updated"},
