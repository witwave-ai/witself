...
function fixture(t) {
  ...
  const sandbox = {
    ...
    fetch(url, options) {
      check(url === "/api/summary" || typeof url === "string" && /^(?:\/api\/self|\/api\/facts\?limit=100|\/api\/facts\/fact_(?:alpha|beta|locked)\/history\?subject=synthetic%20(?:alpha|beta|locked)&predicate=sample%2Fvalue)$/.test(url), "unexpected fetch URL");
  ...
  const h = {
    ...
    begin(hash, urls) {
      ...
      assert.deepEqual(added.map((request) => request.url), hash === "#/overview" ? ["/api/summary", ...urls] : urls, "fact navigation fixture: route request inventory");
...
test("fact navigation renders current inventory and filter", options, async (t) => {
  ...
  assert.equal(h.requests.length, 3, "filter and locked controls make no additional request");
...
test("fact navigation renders current warm detail and history", options, async (t) => {
  ...
  assert.equal(h.requests.length, 4, "warm detail does not fetch another inventory");
  t.diagnostic("fact navigation current baseline completed");
});
...
test("fact navigation renders current cold detail and history", options, async (t) => {
  ...
  assert.equal(h.requests.length, 4, "cold detail fetches exactly inventory then addressed history");
  t.diagnostic("fact navigation current baseline completed");
});
...
test("fact navigation surfaces missing current fact", options, async (t) => {
  ...
  assert.equal(h.requests.length, 3, "missing selected fact does not broaden inventory or fetch history");
  t.diagnostic("fact navigation current baseline completed");
});
