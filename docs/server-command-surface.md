...
| `WITSELF_PROVISION_TOKEN` | Dedicated control-plane-to-cell credential for account provisioning and lifecycle operations. It must not be reused as the backup credential. |
| `WITSELF_BACKUP_TOKEN` | Dedicated control-plane-to-cell credential for read-only logical account backup export. It must be distinct from `WITSELF_PROVISION_TOKEN`; the provisioning credential is never accepted on backup routes. The server chart reads it from `backup.existingSecret`; the managed app-of-apps sets that reference only when `apps.witselfServer.backup.enabled=true`. |
...
| `WITSELF_AGENT_EMAIL_CELL_STORAGE_ADMISSION_BYTES` | Cell-local logical retained-email charge at which new inbound and outbound roots stop. Default: `3221225472` (3 GiB). Schema 91 still permits reserved lifecycle rows and charge-neutral lifecycle updates until the independent hard boundary. This is platform safety, not an account or plan allowance. |
...
| `WITSELF_AGENT_EMAIL_RATE_BUCKET_CLEANUP_ENABLED` | Enable stale agent-email rate-bucket cleanup in `witself-worker`. It covers inbound realm/account debt and outbound minute/daily/recipient debt across three physical tables. Default: `true`; API deployments never run the job. Every worker replica may enable it because the delete batches use `FOR UPDATE SKIP LOCKED`. |
| `WITSELF_AGENT_EMAIL_RATE_BUCKET_CLEANUP_BATCH_SIZE` | Maximum stale agent-email rate-bucket rows deleted by one database batch. Default: `10000`; valid range: `1`-`10000`. |
| `WITSELF_AGENT_EMAIL_RATE_BUCKET_CLEANUP_INTERVAL` | Delay between agent-email rate-bucket cleanup sweeps. Default: `1m`; valid range: `1m`-`24h`. |
| `WITSELF_AGENT_EMAIL_RATE_BUCKET_CLEANUP_BATCH_TIMEOUT` | Shared deadline for one bounded agent-email rate-bucket cleanup sweep. Default: `10s`; valid range: `1s`-`5m`. |
