...
- Independent enabled-by-default cleanup of expired agent-email rate
...
use an existing Secret reference or cloud workload identity where supported.
Infrastructure KMS custody is configured outside the agent-vault contract;
the chart has no application KMS provider setting.
...
The chart must never write an AVK, an unwrapped
DEK, or any infrastructure KMS credential into a ConfigMap, log line, metric label, annotation,
