...
| Bounds / abuse | **CONDITIONAL** | Subjects, bodies, pages, roles, states, and value exposure are bounded; closed-account purge deletes tickets after the 30-day closure grace. API/CLI ticket creation is rate-limited per account; a default-off bounded worker resolves stale awaiting-customer tickets after a human reply. Attachment handling remains absent. [values.yaml](../.gitops/cells/civo-sandbox-usw2-dev/values.yaml), [data-retention-policy.md](../docs/data-retention-policy.md), [security-policy.md](../docs/security-policy.md), [index.js](../infra/cloudflare/support-email-intake/src/index.js), [support.go](../internal/store/support.go), [support_age_out_test.go](../internal/store/support_age_out_test.go), [support_email.go](../internal/store/support_email.go), [support_limits_test.go](../internal/store/support_limits_test.go) |
...
- `support-abuse-retention` (bounds / abuse): Retain PostgreSQL admission and age-out validation and add bounded evidence-attachment handling; per-account API/CLI rate limiting and a default-off bounded awaiting-customer age-out worker are implemented. ([tracking/evidence](../internal/store/support.go))
