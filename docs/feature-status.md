...
- Detailed docs: [backup-and-recovery.md](../docs/backup-and-recovery.md), [data-retention-policy.md](../docs/data-retention-policy.md), [deployment-cells.md](../docs/deployment-cells.md), [governance-and-support.md](../docs/governance-and-support.md)
...
| Observability | **CONDITIONAL** | The replacement serving cell enables scraping, PVC metrics, routing, and PagerDuty/dead-man receivers in GitOps. Fresh recovery acceptance is required; the former cell's evidence does not cover it. Monitoring on other accepting and backup cells still gates broad movement. [values.yaml](../.gitops/cells/civo-sandbox-use1-serving/values.yaml), [launch-readiness.md](../docs/launch-readiness.md), [observability-and-operations.md](../docs/observability-and-operations.md) |
...
- `continuous-capacity-alerting` (observability): Serving-cell capacity scraping and alert delivery are deployed and accepted; extend monitoring coverage to every accepting destination cell before automatic placement resumes. ([tracking/evidence](../docs/observability-and-operations.md))
...
The local Agent Console keeps seven agent sections and adds a conditional, read-only Account area in browser and TUI for the verified current same-account manager: profile, this-device clients, role-filtered plan/billing, support, and access. Agent authority is unchanged.
...
| Gate | State | Current evidence and conclusion |
|---|---|---|
| Behavior | **PASS** | Seven agent sections remain intact. The Account extension uses a separate current same-account manager, six role-filtered read subsections, explicit local client checks, and selected support text. [README.md](../README.md), [dashboard_test.go](../cmd/witself/dashboard_test.go), [account_test.go](../internal/agenttui/account_test.go), [account_test.go](../internal/dashboard/account_test.go), [account_ui_test.go](../internal/dashboard/account_ui_test.go), [dashboard_test.go](../internal/dashboard/dashboard_test.go) |
| Entitlement / policy | **PASS** | Agent panels use the agent token and cell-applied entitlement snapshot. Account reads independently recheck the original current CLI manager for the same account; explicit endpoint/token-file stays agent-only. No agent grant, fleet/admin privilege, or domain mutation is added. [plan_entitlements_test.go](../cmd/witself-server/plan_entitlements_test.go), [agent-console.md](../docs/agent-console.md), [0004-local-agent-dashboard.md](../docs/decisions/0004-local-agent-dashboard.md), [account_test.go](../internal/dashboard/account_test.go), [dashboard_test.go](../internal/dashboard/dashboard_test.go), [self_test.go](../internal/server/self_test.go) |
| Bounds / abuse | **PASS** | Loopback/auth guards and allowlisted projections remain. Account adds bounded reads, fixed resource errors, ephemeral selected support text, exact money/unknown states, and explicit metadata-only local checks. Manager bearers and routine manager-session URLs remain private. [0004-local-agent-dashboard.md](../docs/decisions/0004-local-agent-dashboard.md), [inventory_test.go](../internal/clientinventory/inventory_test.go), [account_test.go](../internal/dashboard/account_test.go), [dashboard_test.go](../internal/dashboard/dashboard_test.go), [self_test.go](../internal/server/self_test.go) |
...
| Docs / support | **PASS** | Browser and TUI guides cover conditional Account access, role-filtered reads, explicit local check limits, support transport/display caps, current-manager session matching, private manual opening, and the unchanged agent/domain boundary. [README.md](../README.md), [agent-console.md](../docs/agent-console.md), [agent-tui.md](../docs/agent-tui.md), [cli-command-surface.md](../docs/cli-command-surface.md), [0004-local-agent-dashboard.md](../docs/decisions/0004-local-agent-dashboard.md) |
...
- Managed rollout: `limited`
- Readiness: **conditional**
- Retained release/cohort evidence: `v0.0.258` — Exact Founder dispatch deployment at v0.0.258/schema 93 (dual-cell wave rollout postflight, outbound-staleness alerting live); a fresh provider-delivery canary remains open.
...
| Observability | **CONDITIONAL** | Workers Observability, queue metrics, lifecycle events, cell metrics, storage gauges, logs, and health exist; cell-side outbound staleness, worker-job, storage, and PVC alerts route to a tested PagerDuty receiver, while Cloudflare dispatch queue/DLQ and delivery-outcome metrics remain unconnected to alerting. [founder-open-plane.rules.yaml](../.gitops/charts/platform/files/founder-open-plane.rules.yaml), [agent-email.md](../docs/agent-email.md), [launch-readiness.md](../docs/launch-readiness.md), [observability-and-operations.md](../docs/observability-and-operations.md) |
| Recovery | **CONDITIONAL** | Idempotent send records, queue retries, receipts, lifecycle reconciliation, DLQ, retention, and archive/import are implemented; populated schema-93 email movement and restore remain unproven. [agent-email.md](../docs/agent-email.md), [README.md](../infra/cloudflare/agent-email-send/README.md) |
| Rollout / canaries | **CONDITIONAL** | Release v0.0.258 (schema 93) is live on both cells for the Founder cohort with dispatch and lifecycle events enabled, but broad enrollment and a fresh end-to-end delivery canary remain open. [agent-email.md](../docs/agent-email.md), [launch-readiness.md](../docs/launch-readiness.md), [tag #v0.0.258](https://github.com/witwave-ai/witself/releases/tag/v0.0.258) |
...
- `continuous-delivery-alerting` (observability): Connect Cloudflare dispatch queue/DLQ and provider-outcome metrics to the now-live tested receiver; storage and cell-worker alerts are already routed. ([tracking/evidence](../docs/observability-and-operations.md))
- `email-move-recovery-canary` (recovery): Move and restore populated schema-93 email state, including accepted/delivered sends, provider events, suppressions, claims, and provider-started refusal. ([tracking/evidence](../docs/cell-worker.md))
...
- Managed rollout: `limited`
- Readiness: **not ready**
- Detailed docs: [audit-retention.md](../docs/audit-retention.md), [cli-command-surface.md](../docs/cli-command-surface.md), [data-retention-policy.md](../docs/data-retention-policy.md)
...
| Rollout / canaries | **CONDITIONAL** | All managed activation gates remain dark and no customer domain routes mail to an agent. [agent-email.md](../docs/agent-email.md), [assert-custom-domain-dark.mjs](../infra/cloudflare/control-plane/scripts/assert-custom-domain-dark.mjs) |
...
| Rollout / canaries | **CONDITIONAL** | Core behavior is released and permanent fact deletion is enabled on both cells (#312). Retained current-release preview/apply/isolation acceptance remains open. [values.yaml](../.gitops/cells/civo-sandbox-use1-backup/values.yaml), [values.yaml](../.gitops/cells/civo-sandbox-use1-serving/values.yaml), [fact-service.md](../docs/fact-service.md), [launch-readiness.md](../docs/launch-readiness.md), [runbooks.md](../docs/runbooks.md) |
...
Signed releases, immutable images, Helm/GitOps cells, schema checks, encrypted restore-verified backups, archive validation, recovery runbooks, and continuous serving-cell monitoring with a tested external receiver are implemented; broad mobility, provider PITR, and multi-cell movement drills remain constrained.

- Implementation: `implemented`
- Managed rollout: `limited`
- Readiness: **conditional**
- Retained release/cohort evidence: `v0.0.258` — Signed v0.0.258 release, dual-cell restore-verified pre-migration backups, waved rolls of both Civo cells to schema 93, CP and edge attested at 0.0.259, and accepted serving-cell monitoring; broad placement and movement drills remain open.
...
| Observability | **CONDITIONAL** | The replacement cell enables scraping, PVC metrics, routing, and PagerDuty/dead-man receivers. Former-cell acceptance (#267) does not cover recovery. The chart has 31 rules, including five product-family rules with promtool tests (#357); live loading and backup/future-cell monitoring need retained proof. [values.yaml](../.gitops/cells/civo-sandbox-use1-serving/values.yaml), [founder-open-plane.rules.yaml](../.gitops/charts/platform/files/founder-open-plane.rules.yaml), [founder-open-plane.rules.test.yaml](../.gitops/charts/platform/testdata/founder-open-plane.rules.test.yaml), [launch-readiness.md](../docs/launch-readiness.md), [observability-and-operations.md](../docs/observability-and-operations.md), [commit #50976cadc1816c1a2f37a92e2a818cb0218a1eac](https://github.com/witwave-ai/witself/commit/50976cadc1816c1a2f37a92e2a818cb0218a1eac) |
| Recovery | **CONDITIONAL** | Pre-migration backups and restore validation exist. Daily age-encrypted PostgreSQL dumps to R2 and staleness/failure alerts are implemented, default off. Credentialed activation, scheduled restore evidence, provider PITR, production restore, and multi-cell movement drills remain incomplete. [civo-postgres-backup.yaml](../.gitops/charts/apps/templates/civo-postgres-backup.yaml), [postgres-backup.rules.yaml](../.gitops/charts/platform/files/postgres-backup.rules.yaml), [backup_evidence_cmd.go](../cmd/witself-admin/backup_evidence_cmd.go), [backup-and-recovery.md](../docs/backup-and-recovery.md), [deployment-cells.md](../docs/deployment-cells.md) |
...
Account, realm, agent, token, role, suspension, and transactional realm/agent/operator-seat capacity enforcement form the shared identity spine; managed evidence is narrow, operator-seat caps are set per plan (1/3/25, Enterprise unlimited), and the legacy total-agent cap remains a compatibility floor.
...
| Entitlement / policy | **PASS** | Resolved realms, agents, agents-per-realm, and operator-seat limits are transactionally enforced per account (creates refuse past the seat cap; downgrades refuse while headcount exceeds the target plan) with audited overrides and explicit unlimited semantics. [billing-and-limits.md](../docs/billing-and-limits.md), [operator_seats_integration_test.go](../internal/store/operator_seats_integration_test.go), [resource_limit_integration_test.go](../internal/store/resource_limit_integration_test.go) |
...
| Observability | **CONDITIONAL** | Audit, usage, capacity, token, and lifecycle state are inspectable, and a tested external alert path now exists on the monitoring plane, but no unified identity-capacity dashboard or identity-specific alert rules are connected to it. [founder-open-plane.rules.yaml](../.gitops/charts/platform/files/founder-open-plane.rules.yaml), [launch-readiness.md](../docs/launch-readiness.md), [observability-and-operations.md](../docs/observability-and-operations.md) |
...
| Bounds / abuse | **CONDITIONAL** | Subjects, bodies, pages, roles, states, and value exposure are bounded; closed-account purge deletes tickets after the 30-day closure grace. API/CLI ticket creation is rate-limited per account; a default-off bounded worker resolves stale awaiting-customer tickets after a human reply. Attachment handling remains absent. [values.yaml](../.gitops/cells/civo-sandbox-use1-serving/values.yaml), [data-retention-policy.md](../docs/data-retention-policy.md), [security-policy.md](../docs/security-policy.md), [support-admission-age-out-evidence.md](../docs/support-admission-age-out-evidence.md), [index.js](../infra/cloudflare/support-email-intake/src/index.js), [support.go](../internal/store/support.go), [support_email.go](../internal/store/support_email.go), [support_limits_test.go](../internal/store/support_limits_test.go) |
...
| Docs / support | **PASS** | The managed support policy is published: plan entitlement, channels with launch-status notes, a one-business-day first-response promise, AI-first staffing with a fixed human-escalation set, severities, refund/security/deletion paths, and incident communications; the AI runner is documented separately. [governance-and-support.md](../docs/governance-and-support.md), [self-host-support.md](../docs/self-host-support.md), [support-policy.md](../docs/support-policy.md), [support-runner.md](../docs/support-runner.md) |
...
| Observability | **CONDITIONAL** | Value-free memory, curation, worker, and quality evidence schemas exist, and the serving cell has continuous scrape plus generic server/worker alerting to a tested PagerDuty/dead-man receiver; memory/curation-specific SLO rules and retained multi-environment SLO evidence remain absent. [founder-open-plane.rules.yaml](../.gitops/charts/platform/files/founder-open-plane.rules.yaml), [launch-readiness.md](../docs/launch-readiness.md), [memory-load-quality.md](../docs/memory-load-quality.md), [observability-and-operations.md](../docs/observability-and-operations.md) |
...
| Rollout / canaries | **CONDITIONAL** | The receive edge alias gate remains off and no managed customer alias delivers mail. [agent-email.md](../docs/agent-email.md), [assert-custom-domain-dark.mjs](../infra/cloudflare/control-plane/scripts/assert-custom-domain-dark.mjs) |
