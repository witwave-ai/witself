...
## Client-Custodied Vault Storage
...
repository. Infrastructure KMS credentials belong to the infrastructure
identity, not the agent vault. The server needs no KMS unwrap authority for
agent secrets.
...
- The active client encrypts secret values and TOTP seeds with per-field DEKs
  ...
  reveal gates value-returning operations. See [Client-Custodied Vault Storage](#client-custodied-vault-storage),
  [Sealed-Plane Encryption Storage](#sealed-plane-encryption-storage), and
