...
- The API-side gap ("agents forget between runs") is exactly Witself's wedge: a
  portable, vendor-neutral identity store reachable over MCP and HTTP, with
  optional client-supplied memory and query vectors under client-defined profiles.
  The backend never generates embeddings. See
...
client cryptography, infrastructure KMS adapters, authorization and policy
...
Native memory locks an agent's self to one vendor. Keep optional embeddings
client-generated, with clients supplying memory and query vectors under an
immutable vector profile. Lexical and structured recall remain available
without vectors; the backend never calls an embedding model. Portability is a
selling point against OpenAI/Anthropic native memory. See
[memory-model.md](memory-model.md).
