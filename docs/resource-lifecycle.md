...
| Agent | Ordinary: `POST /v1/realms/{realm}/agents`; explicit email segment (v0.0.241+): `POST /v1/realms/{realm}/agents:with-email-segment`; `witself agent create` | `DELETE /v1/realms/{realm}/agents/{agent}`, `witself agent delete --yes` | Production-cohort creation atomically provisions the canonical mailbox. The versioned explicit-segment route fails 404 without mutation on older servers. Soft-delete immediately revokes live agent tokens. |
