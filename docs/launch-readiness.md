...
| **Billing** | ✅ CUTOVER COMPLETE 2026-08-30/31: full pipeline proven live with $0 spend — promo-code Checkout purchase (WITSELF-LAUNCH-PROOF, 100%-off once), webhook→entitlement in 3s, $0 invoice paid, period-end downgrade scheduled and verified (next charge: none); durable-mutation serializer proven under concurrent operations. Stripe Tax LIVE 2026-08-30: CO head office, SaaS-business-use preset, zero registrations (correct pre-nexus; threshold monitoring armed), CP_STRIPE_AUTOMATIC_TAX=true staged + activated; per-product personal/business tax-code split deferred to first registration | GA flag only | 🔑 flip WITSELF_CP_BILLING_GENERAL_AVAILABILITY (needs empty allowlist — Scott decision); (opt) revoke stale Jul-5 secret key |
