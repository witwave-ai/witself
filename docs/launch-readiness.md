...
| **Billing** | ✅ CUTOVER COMPLETE 2026-08-30/31: full pipeline proven live with $0 spend — promo-code Checkout purchase (WITSELF-LAUNCH-PROOF, 100%-off once), webhook→entitlement in 3s, $0 invoice paid, period-end downgrade scheduled and verified (next charge: none); durable-mutation serializer proven under concurrent operations | GA flag + Tax | 🔑 flip WITSELF_CP_BILLING_GENERAL_AVAILABILITY (needs empty allowlist — Scott decision); Stripe Tax registrations + stage CP_STRIPE_AUTOMATIC_TAX=true (bridge mapping landed dark in #304); (opt) revoke stale Jul-5 secret key |
