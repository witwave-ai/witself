...
| **Billing** | ✅ GENERAL AVAILABILITY 2026-08-31 — billing lane COMPLETE: cutover proven at $0 (promo Checkout, webhook→entitlement 3s, $0 invoice, verified period-end downgrade), Stripe Tax live (CO, SaaS preset, zero registrations pre-nexus, threshold monitoring armed), GA flipped at CP v0.0.267 (allowlist deleted, CP_BILLING_GENERAL_AVAILABILITY=true, activation-verified — the container refuses GA alongside a cohort, so activation is the proof). Witself is fully self-service: anyone can sign up and pay | — | ✅ (opt: revoke stale Jul-5 secret key; real-charge proof arrives with the first customer purchase) |
