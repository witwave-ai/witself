...
test("bridge path classifiers are exact", () => {
  ...
  assert.ok(matchAdminPolicyPath(
    "/v1/admin/accounts/acct_1/messaging",
  ));
  assert.ok(matchAdminPolicyPath(
...
test("admin proxy replaces caller credentials and relays Go response", async () => {
  const request = new Request(
    "https://self.witwave.ai/v1/admin/accounts/acct_1/limit-overrides/stored_memory",
    {
      method: "PUT",
      headers: {
...
test("admin proxy routes all message-rate limit dimensions exactly", async () => {
  for (const dimension of [
    "message_sent_per_agent_minute",
    "message_delivered_per_realm_minute",
    "message_delivered_per_recipient_minute",
