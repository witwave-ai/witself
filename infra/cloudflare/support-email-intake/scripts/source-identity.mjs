...
export function assertReleaseSource(identity) {
  if (!identity || typeof identity !== "object" ||
    ...
    throw new Error(
      "support email intake Worker deployment requires a clean checkout at one exact semantic-version tag",
    );
  }
  return identity;
}
...
export function sourceIdentity({
  ...
  const commit = requiredGit(
    ["rev-parse", "HEAD"], repositoryRoot, "source commit", environment,
  );
  if (!FULL_COMMIT.test(commit)) throw new Error("git returned an invalid source commit");
  ...
  if (!validCommitDate(date)) throw new Error("git returned an invalid source commit date");
  ...
  const semanticTags = pointedTags.stdout.split("\n")
  ...
  if (!SAFE_DEVELOPMENT_VERSION.test(version)) {
    throw new Error("could not derive a safe support email intake Worker version");
  }
