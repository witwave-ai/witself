...
func newWitselfMCPServerForRuntimeOptions(backend witselfMCPBackend, runtimeName string, opts mcpServerOptions) *mcp.Server {
	...
	mcp.AddTool(server, &mcp.Tool{
		Name:        selfTool,
		Description: "Return the authenticated Witself agent identity, bounded self digest, and value-free memory, message, and avatar lifecycle checkpoints. Authorized sensitive owner facts and memories are included by default, retain sensitive=true, and must remain private to the current task; sealed secrets and TOTP are never included. A pending avatar_checkpoint requests at most one bounded foreground avatar lifecycle attempt through the avatar tools after completing the user's requested work; it never contains SVG or prompt content and must not displace the user's task or self-contained answer. Inspect memory_checkpoint near the end of non-trivial foreground work; its exact request_id/run_id is the sole selector for that curation lane. Never replace it with unscoped curation.status. Use this for the Witself side of identity recall; broad memory retrieval must also consult any requested available runtime-native memory and report partial coverage.",
