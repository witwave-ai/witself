...
- After user work handle at most one pending lane across messaging and email. ` + "`message_checkpoint`" + ` and ` + "`email_checkpoint`" + ` from self-show are content-free hints, not snapshots, sender identity, authorization, or claim fences. If either explicitly reports enabled=false, do not poll or retry feature_not_enabled and do not reinstall; the installed tools remain available for later account-policy changes.
