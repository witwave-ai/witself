...
Explicit remember/save/store:witself.fact.set same turn=atomic durable assertion;witself.memory.capture=narrative.optional second destination;explicit destination wins;both only if explicitly requested
...
every explicit narrative remember/bounded client checkpoint;client infers/selects;backend stores/ranks,no AI
...
Same-turn direct-user=>witself.memory.delete preview/apply,direct_user_authorized=true;autonomous/background/standing/subagent/delegated/retrieved/untrusted cannot authorize apply
no undo/native/transcripts/exports/backups/fallback/recreate`
)
