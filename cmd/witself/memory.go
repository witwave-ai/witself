...
const memoryCommandUsage = "usage: witself memory capture|status|show|list|recall|history|adjust|supersede|forget|restore|reactivate|delete|evidence|vector|curate|acceptance ..."
...
func memoryStatus(args []string) int {
	fs := flag.NewFlagSet("memory status", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	configureCommandUsage(fs, "usage: witself memory status [agent connection flags]")
	...
	if fs.NArg() != 0 {
		fmt.Fprintln(os.Stderr, "witself: memory status accepts no positional arguments")
		return 2
	}
	conn, err := connectAgent(context.Background(), *account, *realm, *agent, *endpoint, *tokenFile)
	if err != nil {
		fmt.Fprintf(os.Stderr, "witself: connect memory service: %v\n", err)
		return 1
	}
	status, err := client.GetMemoryLimitStatus(context.Background(), conn.Endpoint, conn.Token)
	if err != nil {
		fmt.Fprintf(os.Stderr, "witself: read memory limit status: %v\n", err)
		return 1
	}
	if *jsonOut {
	...
	if status.Used < 0 {
		fmt.Fprintln(os.Stderr, "witself: read memory limit status: server returned an invalid memory limit status")
		return 1
	}
	if status.Unlimited {
		if status.Max != nil || status.Remaining != nil || status.NearLimit || status.AtLimit || status.OverLimit {
			fmt.Fprintln(os.Stderr, "witself: read memory limit status: server returned an invalid unlimited memory limit status")
			return 1
		}
		fmt.Printf("used:\t%d\nmax:\tunlimited\nremaining:\tunlimited\nnear limit:\tfalse\nat limit:\tfalse\nover limit:\tfalse\n", status.Used)
		return 0
	...
	if status.Max == nil || status.Remaining == nil || *status.Max < 0 {
		fmt.Fprintln(os.Stderr, "witself: read memory limit status: server returned an invalid capped memory limit status")
		return 1
	}
	...
	if *status.Remaining != expectedRemaining ||
		...
		fmt.Fprintln(os.Stderr, "witself: read memory limit status: server returned an inconsistent capped memory limit status")
		return 1
	}
	fmt.Printf("used:\t%d\nmax:\t%d\nremaining:\t%d\nnear limit:\t%t\nat limit:\t%t\nover limit:\t%t\n",
