...
- For a specific fact lookup, query the relevant Witself fact tools first. Query Witself narrative memory when the request indicates that narrative history may matter. Consult Codex native-memory context only when the user explicitly names Codex memory or asks for all sources. Keep sensitive facts redacted by default; reveal one only for an exact, intentional lookup when the authenticated user needs the value. When revealing it, put the requested value in the final answer, not only in tool output or hook context.
