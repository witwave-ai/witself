...
func writeFakeGrokInstallCLI(t *testing.T, path string) {
	t.Helper()
	statePath := path + ".mcp-state"
	t.Setenv("FAKE_GROK_MCP_STATE", statePath)
	script := `#!/bin/sh
...
if [ "$1 $2 $3" = "mcp list --json" ]; then
  if [ ! -f "$FAKE_GROK_MCP_STATE" ]; then
    printf '%s\n' '[]'
    exit 0
  fi
  set -- $(cat "$FAKE_GROK_MCP_STATE")
  shift 6
  command="$1"
  shift
